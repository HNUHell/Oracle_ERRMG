...
* **措施**: 增加**PROCESSES**初始化参数的值。【是否需要增加**PROCESSES**值还需进行判断，是否是由于此值过小而现实场景需要更大的值？】
  ...
  - 如果判断确定是由于**PROCESSES**值过小导致，则需修改增大此参数值，【详情参见ORA-00018=>原因1=>措施：[gitbook](#原因1-所有会话状态对象都在使用中。【部分验证】)||[github](#原因1-所有会话状态对象都在使用中部分验证)，将SQL语句中``name = 'sessions'``修改为``name = 'processes'``即可】 
